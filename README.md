...
> 💡 **註冊機制與密碼重設**：
> - **邀請碼註冊系統**：為確保系統安全性，目前「註冊新帳號」預設僅能註冊為 **「學員 (Student)」** 角色。若需註冊為教師、牧者、家長或中央管理員，必須先由現有管理員登入「Admin 控制面板」產生對應角色的**邀請碼 (Invite Code)**。被邀請者在註冊時輸入邀請碼，系統將自動鎖定其角色與教會。
> - **重設密碼功能**：若使用者忘記密碼，管理員（Admin）可於控制面板的「使用者帳號管理」中協助將密碼重置為預設值。
